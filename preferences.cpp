...
void Preference::ShowData(int pos)
{
    if(pos == ACCOUNT)
    {
        ...
        ui.spaceLabel->setText(tr("用户空间总量:")+QString::number(totalInt)+"GB");
        ...
        QString usedSpace =tr("已经使用的空间:")+QString::number(usedInt)+unit;
        ui.usedSpaceLabel->setText(usedSpace);
        int maxFileSize = userInfo["max_file_size"].toLongLong()/1024/1024;
        QString maxFileSizeStr =tr("允许上传最大文件:")+QString::number(maxFileSize)+"MB";
...
void Preference::Unlink()
{
    ...
    ui.spaceLabel->setText(tr("用户空间总量:")+"0 GB");
    QString maxFileSizeStr =tr("允许上传最大文件:")+"0 MB";
    ui.maxFileSizeLabel->setText(maxFileSizeStr);
    QString usedSpace =tr("已经使用的空间:")+" 0";
    ui.usedSpaceLabel->setText(usedSpace);
    emit removeToken();
}
...
{
    QString dir = QFileDialog::getExistingDirectory(this,tr("选择目录"),qgetenv("HOME"));
